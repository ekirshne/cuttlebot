...
class ColorFilter():
    ...
    def set_filter(self, hue: int, precision: int):
        ...
        if hue - precision < 0:
            self.lower_bound_filter_range = (np.uint8([(hue-precision)+180, 160, 60]), np.uint8([180, 255, 255]))
            self.upper_bound_filter_range = (np.uint8([0, 160, 60]), np.uint8([hue+precision, 255, 255]))
        #Set the filter for hue ranges wrapping from 180 -> 0
        elif hue + precision >= 180:
            self.lower_bound_filter_range = (np.uint8([hue-precision, 160, 60]), np.uint8([180, 255, 255]))
            self.upper_bound_filter_range = (np.uint8([0, 160, 60]), np.uint8([(hue+precision)-180, 255, 255]))
        #Set the filter for hue ranges when there is no wrapping involved
        else:
            self.lower_bound_filter_range = (np.uint8([hue-precision, 160, 60]), np.uint8([hue, 255, 255]))
            self.upper_bound_filter_range = (np.uint8([hue, 160, 60]), np.uint8([hue+precision, 255, 255]))
